...
func update_animations(move_direciton):
	if (move_direciton != Vector2.ZERO):
		animationa_playback.travel("caminar") #caminar
		animation_tree.set("parameters/caminar/blend_position", move_direciton)
		animation_tree.set("parameters/Reposo/blend_position", move_direciton)
	else:
